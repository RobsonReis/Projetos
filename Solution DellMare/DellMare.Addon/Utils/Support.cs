    ...
    static public class Support
    {
        ...
        public static string GetValChooseFromList(SAPbouiCOM.ItemEvent evento, string ChooseFromList_ID, string Coluna, int Linha)
        {
            try
            {
                ...
                SAPbouiCOM.Form oForm = B1Connections.theAppl.Forms.GetFormByTypeAndCount(evento.FormType, evento.FormTypeCount);
        ...
        static public FileInfo arquivo(string nomeArquivo)
        {
            string folder = "select U_Folder from [@PARAMETROS]";
            folder = (string)B1Connections.ExecuteSqlScalar(folder);
            DirectoryInfo di = new DirectoryInfo(folder);
            FileInfo[] fi = di.GetFiles("*.xlsx");

            foreach (FileInfo arquivo in fi)
            {
                if (Path.GetFileNameWithoutExtension(arquivo.FullName) == nomeArquivo)
                {
                    return arquivo;
                }
            }
